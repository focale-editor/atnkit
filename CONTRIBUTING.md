# Contributing to AtnKit

Thanks for helping improve AtnKit. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/atnkit/issues) before opening a new one.
...
AtnKit follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Dart SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/atnkit.git
cd atnkit
dart pub get
```
...
ATN codec changes should include focused round-trip, malformed-input, descriptor-boundary, preservation, and resource-limit tests where applicable. Do not add a guessed recovery rule for an unknown binary structure: preserve data only when its record boundary is known, and fail explicitly otherwise.
...
Add any narrower corpus or integration checks relevant to the files you changed. If a check cannot run in your environment, explain why in the pull request.
...
Add `!` for an accepted breaking change, for example `feat!: replace the event model`.
